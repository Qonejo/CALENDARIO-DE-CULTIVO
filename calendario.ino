...
void validarInicioFases() {
  diaInicioVeg = constrain(diaInicioVeg, 0, 239);
  diaInicioFlor = constrain(diaInicioFlor, 1, 240);
  if (diaInicioVeg >= diaInicioFlor) {
    diaInicioFlor = constrain(diaInicioVeg + 1, 1, 240);
    if (diaInicioFlor <= diaInicioVeg) diaInicioVeg = diaInicioFlor - 1;
  }
}

int mesDesdeTexto(const char* m) {
  if (!strcmp(m, "Jan")) return 0; if (!strcmp(m, "Feb")) return 1; if (!strcmp(m, "Mar")) return 2;
  if (!strcmp(m, "Apr")) return 3; if (!strcmp(m, "May")) return 4; if (!strcmp(m, "Jun")) return 5;
  if (!strcmp(m, "Jul")) return 6; if (!strcmp(m, "Aug")) return 7; if (!strcmp(m, "Sep")) return 8;
  if (!strcmp(m, "Oct")) return 9; if (!strcmp(m, "Nov")) return 10; if (!strcmp(m, "Dec")) return 11;
  return 0;
}
...
InfoCultivo calcularCultivo(int d, int m, int a) {
  ...
  if (diasVida < diaInicioVeg) {
    r.fase = "PLANTULA";
    r.colorFase = MI_CIAN;
    ppmMin = 100; ppmMax = 250;
    inicioFase = 0; finFase = diaInicioVeg;
    diasRel = diasVida;
  } else if (diasVida < diaInicioFlor) {
    r.fase = "VEGETA";
    r.colorFase = MI_VERDE;
    ppmMin = 300; ppmMax = 450;
    inicioFase = diaInicioVeg; finFase = diaInicioFlor;
    diasRel = diasVida;
  } else {
    ...
    if (diasFlor <= 47) {
      ...
      inicioFase = 0; finFase = 47;
      diasRel = diasFlor;
    } else {
      r.fase = "FLOR A";
      r.colorFase = MI_ROJO;
      ppmMin = 1200; ppmMax = 1600;
      inicioFase = 48; finFase = 68;
      diasRel = diasFlor;
    }
  }

  float prog = (float)(diasRel - inicioFase) / max(1, (finFase - inicioFase));
...
void dibujarSelectorConfig() {
  ...
  uint16_t colVolver = (indiceCfg == 2) ? MI_NARANJA : MI_GRIS;
...
void dibujarConfiguracion() {
  ...
  tft.setTextSize(1); tft.setCursor(10, 48); tft.print("WiFi:"); tft.setTextColor(wifiConectado ? MI_VERDE : MI_ROJO);
  tft.setCursor(50, 48); tft.print(wifiConectado ? "Conectado" : "Sin conexion");

  tft.setTextColor(MI_BLANCO); tft.setCursor(10, 84); tft.print("Inicio VEG:");
  tft.setCursor(10, 120); tft.print("Inicio FLOR:");
  ...
  tft.setTextSize(2); tft.setTextColor(MI_BLANCO); tft.setCursor(VEG_VAL_X + 10, VEG_VAL_Y + 6); tft.printf("%d", diaInicioVeg);
  ...
  tft.setCursor(FLOR_VAL_X + 10, FLOR_VAL_Y + 6); tft.printf("%d", diaInicioFlor);
...
void aplicarPasoEncoder(int8_t dir) {
  if (dir == 0) return;
  switch (estadoUI) {
    case UI_CALENDARIO: offsetVer += dir; break;
    case UI_CONFIG: indiceCfg = (indiceCfg + (dir > 0 ? 1 : -1) + 3) % 3; break;
    case UI_EDIT_VEG: diaInicioVeg += dir; validarInicioFases(); break;
    case UI_EDIT_FLOR: diaInicioFlor += dir; validarInicioFases(); break;
  }
...
void actualizarEncoder() {
  ...
  int clk = digitalRead(ENC_S1), dt = digitalRead(ENC_S2);
  if ((clk != ultimoCLK || dt != ultimoDT) && (ahora - ultimoMsEncoder >= DEBOUNCE_ENCODER_MS)) {
    ...
    int8_t paso = 0;
    if (trans == 0b1101 || trans == 0b0100 || trans == 0b0010 || trans == 0b1011) paso = 1;
    else if (trans == 0b1110 || trans == 0b0111 || trans == 0b0001 || trans == 0b1000) paso = -1;
    if (paso != 0 && (ahora - ultimoMsPaso >= STEP_GAP_MS)) { deltaEncoder += paso; ultimoMsPaso = ahora; }
...
void consumirEncoder() {
  ...
  if (!pasos) return;
  deltaEncoder = 0;
  while (pasos > 0) { aplicarPasoEncoder(1); pasos--; }
  while (pasos < 0) { aplicarPasoEncoder(-1); pasos++; }
}
...
void actualizarFechaSiEsPosible() {
  if (WiFi.status() != WL_CONNECTED) {
    wifiConectado = false;
    if (millis() - ultimoIntentoWifiMs > 30000UL) { WiFi.begin(SSID, PASSWORD); ultimoIntentoWifiMs = millis(); }
    return;
  }
  wifiConectado = true;
  struct tm ti;
  if (getLocalTime(&ti, 50)) {
    ...
    if (nd != diaHoy || nm != mesHoy || na != anioHoy) {
      diaHoy = nd; mesHoy = nm; anioHoy = na;
      necesitaRedibujar = true;
    }
    return;
  }
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED && millis() - inicioIntento < 15000) delay(200);
  ...
  if (wifiConectado && getLocalTime(&ti)) {
    ...
    } else { diaHoy = 1; mesHoy = 0; anioHoy = 2024; }
  }
...
void loop() {
  ...
  if (necesitaRedibujar) {
    if (estadoUI == UI_CALENDARIO) dibujarCalendario(); else dibujarConfiguracion();
